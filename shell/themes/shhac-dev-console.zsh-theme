...
#
# Dev Console Theme for Oh My Zsh
# ================================
#
# A terminal-style zsh theme with bracketed sections inspired by developer
# console logs. Displays time, status, path, virtualenv, node version, and git
# information in a clean, structured format with ASCII-style brackets.
#
# Configuration
# -------------
...
# Features
# --------
# - Developer console aesthetic with [ ] brackets
# - Git ahead/behind tracking (↑N ↓N)
# - Unified change indicator (±) instead of separate staged/unstaged
...
# - Performance optimizations with cached command checks
#
# Display Order: time → error/status → context → path → venv → node → git
#
...
prompt_time() {
  ...
  echo -n "%{%F{cyan}%}[ %{%F{242}%}$time_str %{%F{cyan}%}]"
}
...
prompt_status() {
  ...
  [[ -n "$symbols" ]] && echo -n "%{%F{242}%}──%{%F{red}%}[$symbols %{%F{red}%}]"
}
...
prompt_context() {
  ...
  if [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    echo -n "%{%F{242}%}──%{%F{yellow}%}[ %{%F{default}%}%(!.%{%F{yellow}%}.)%n@%m %{%F{yellow}%}]"
  fi
}
...
prompt_dir() {
  ...
  echo -n "%{%F{242}%}──%{%F{blue}%}[ $result %{%F{blue}%}]"
}
...
prompt_virtualenv() {
  ...
  if [[ -n $virtualenv_path && -n $VIRTUAL_ENV_DISABLE_PROMPT ]]; then
    ...
    echo -n "%{%F{242}%}──%{%F{cyan}%}[ $__shhac_theme_venv_icon $venv_name %{%F{cyan}%}]"
  fi
}
...
prompt_node() {
  ...
  if command -v node >/dev/null 2>&1; then
    ...
    echo -n "%{%F{242}%}──%{%F{magenta}%}[ $__shhac_theme_node_icon $nv %{%F{magenta}%}]"
  fi
}
...
prompt_git() {
  ...
  # Build indicators with granular checks
  local indicators=""

  # Change indicators: ± (both), + (staged only), ● (unstaged only)
  if [[ $has_staged -eq 1 && $has_unstaged -eq 1 ]]; then
    indicators="±"
  ...
  echo -n "%{%F{242}%}──${git_color}[ $__shhac_theme_git_icon ${branch}${indicators}${mode} ${git_color}]"
}
