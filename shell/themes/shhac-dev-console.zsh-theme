...
#
# Order: time → error/status → context → path → venv → node → git
...
prompt_virtualenv() {
  ...
  if [[ -n $virtualenv_path && -n $VIRTUAL_ENV_DISABLE_PROMPT ]]; then
    echo -n "%{%F{242}%}──%{%F{cyan}%}[ 🐍 $(basename $virtualenv_path) %{%F{cyan}%}]"
  fi
}
...
prompt_node() {
  ...
  if [ -x "$(command -v node)" ]; then
    ...
    echo -n "%{%F{242}%}──%{%F{magenta}%}[ ⬢ $nv %{%F{magenta}%}]"
  fi
}
...
prompt_git() {
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    echo -n "%{%F{242}%}──${git_color}[ ${branch}${vcs_info_msg_0_%% }${mode} ${git_color}]"
  fi
}
