...
#
# Soft Bubbles Theme for Oh My Zsh
# ================================
#
# A friendly zsh theme with circle separators (◉) displaying time, status, path,
# virtualenv, node version, and git information in a clean, pastel-colored format.
#
# Configuration
# -------------
...
# Features
# --------
# - Friendly circle separator design (◉)
...
prompt_status() {
  ...
  [[ -n "$symbols" ]] && echo -n " %{%F{red}%}◉$symbols"
}
...
prompt_context() {
  ...
  if [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    echo -n " %{%F{yellow}%}◉ %{%F{default}%}%(!.%{%F{yellow}%}.)%n@%m"
  fi
}
