...
#
# Cyberpunk Theme for Oh My Zsh
# ==============================
#
# A bold zsh theme with cyberpunk aesthetics using block characters (▓▒░),
# neon colors, and information-dense display. Inspired by 80s/90s hacker
# terminals and cyberpunk visual novels.
#
# Configuration
# -------------
...
# Features
# --------
# - Cyberpunk block character design (▓▒░)
# - Neon color palette (cyan, magenta, yellow, blue)
...
prompt_time() {
  ...
  echo -n "%{%F{magenta}%}▓▒░ %{%F{cyan}%}$time_str %{%F{magenta}%}░▒▓"
}
...
prompt_context() {
  ...
  if [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    echo -n " %{%F{yellow}%}▓▒░ %{%F{default}%}%(!.%{%F{yellow}%}.)%n@%m %{%F{yellow}%}░▒▓"
  fi
}
...
prompt_virtualenv() {
  ...
  if [[ -n $virtualenv_path && -n $VIRTUAL_ENV_DISABLE_PROMPT ]]; then
    ...
    echo -n " %{%F{cyan}%}▓▒░ $__shhac_theme_venv_icon $venv_name %{%F{cyan}%}░▒▓"
  fi
}
...
prompt_node() {
  ...
  if (( __shhac_has_node )); then
    ...
    echo -n " %{%F{magenta}%}▓▒░ $__shhac_theme_node_icon $nv %{%F{magenta}%}░▒▓"
  fi
}
...
prompt_git() {
  ...
  local git_color border_color
  if [[ $has_staged -eq 1 || $has_unstaged -eq 1 ]]; then
    git_color="%{%F{yellow}%}"
    border_color="%{%F{yellow}%}"
  else
    git_color="%{%F{green}%}"
    border_color="%{%F{green}%}"
  fi
  ...
  if [[ $has_upstream -eq 0 && $is_detached -eq 0 ]]; then
    ...
    [[ $ahead -gt 0 ]] && tracking_indicator+=" ↑$ahead"
    [[ $behind -gt 0 ]] && tracking_indicator+=" ↓$behind"
  fi

  indicators="${indicators}${tracking_indicator}"
  ...
  echo -n " ${border_color}▓▒░ ${git_color}${branch}${indicators}${mode} ${border_color}░▒▓"
}
