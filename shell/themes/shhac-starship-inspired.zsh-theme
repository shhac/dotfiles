...
#
# Starship Inspired Theme for Oh My Zsh
# ======================================
#
# A modern zsh theme with sharp angular box-drawing characters inspired by
# Starship prompt. Displays time, status, path, virtualenv, node version,
# and git information in a clean, information-dense format.
#
# Configuration
# -------------
...
# Features
# --------
# - Sharp angular box-drawing design (┌ └)
...
() {
  if [[ "${SHHAC_THEME_USE_POWERLINE:-true}" == "false" ]]; then
    ...
    typeset -g __shhac_theme_path_icon=''
  else
    ...
    typeset -g __shhac_theme_path_icon='󱃾'
  fi
}
...
prompt_dir() {
  ...
  echo -n " %{%F{blue}%}$__shhac_theme_path_icon $result"
}
