...
#
# Minimal Zen Theme
# Ultra clean with simple pipe separators
#
...
prompt_status() {
  ...
  [[ -n "$symbols" ]] && echo -n " %{%F{242}%}│$symbols"
}
...
prompt_virtualenv() {
  ...
  if [[ -n $virtualenv_path && -n $VIRTUAL_ENV_DISABLE_PROMPT ]]; then
    echo -n " %{%F{242}%}│ %{%F{cyan}%}🐍 $(basename $virtualenv_path)"
  fi
}
...
prompt_git() {
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    echo -n " %{%F{242}%}│ ${git_color}${branch}${vcs_info_msg_0_%% }${mode}"
  fi
}
