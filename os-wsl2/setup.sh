...
export DOTFILES_OS="${DOTFILES_OS:-wsl2}"
...
if prompt_yes_no "Set up WSL2-specific configurations? (Windows interop, X11, aliases)"; then
    ...
    info "WSL2 shell aliases are managed by the wsl2 stow package"
else
...
echo "  2. Restore agent-* CLI config: ./setup.sh --secrets-open, then --reauth"
echo "  3. Restart WSL2 to apply Docker group changes: 'wsl --shutdown' from Windows"
echo "  4. Install Windows Terminal for a better terminal experience"
echo "  5. Consider installing VS Code with WSL extension"
echo ""
